...
export default function FormPage() {
  ...
  useEffect(() => {
    // If the session status is loading, you can show a loading indicator
    if (status === 'loading') {
      // Show a loading state or loading indicator here if needed
      return;
    }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post('/api/intern', data);
        ...
        <div className="bg-gray-200 flex-grow rounded-lg mt-2 mr-3">
          ...
          <div className="justify-between">
            <form onSubmit={handleSubmit} className="pt-5 flex flex-col">
              ...
              <button type="submit" className="btn-default mr-3 btn-small">
                Save
              </button>
            </form>
          </div>
